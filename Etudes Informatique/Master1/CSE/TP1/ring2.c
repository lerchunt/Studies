...
void transmission_valeur(int n){
	...
	else
	{
		...
		close(tpipe[modulo_positif(rang,n)][0]);
		if(nbmax[0] < nb[0]) 
		{
		...
		close(tpipe[modulo_positif(rang,n)][1]);
	}
}

/* Le processus principal (rang = 0) lit la valeur maximale finale dans le dernier tube et affiche le gagnant */
void print_valeur(int n){
	if(rang == 0)
	{
		...
		if(x != sizeof(int)*3){ /* Verifie que la lecture s'est bien passee */
			perror("read : Erreur de lecture2 ");
			exit(-1);
		}
		close(tpipe[modulo_positif(rang-1,n)][0]);
		printf("the winner is %d pid %d node %d \n",nbfinal[0],nbfinal[1],nbfinal[2]);
	}
}
