...
//ajout des nouveaux ennemis
	...
	if( _ennemisToSprite.empty())				//si le map d'ennemis est vide on le remplit
	{
		...
		{
			if ((*it2)->getType()==1)
				_ennemisToSprite[(*it2)] =new EnnemiSprite(&_ennemis_image,(*it2)->getX(),(*it2)->getY(),ENNEMI_WIDTH,ENNEMI_HEIGHT,true,false);
			else if ((*it2)->getType()==2)
				_ennemisToSprite[(*it2)] =new EnnemiSprite(&_ennemis2_image,(*it2)->getX(),(*it2)->getY(),ENNEMI_WIDTH,ENNEMI_HEIGHT,true,false);
			else
				_ennemisToSprite[(*it2)] =new EnnemiSprite(&_ennemis3_image,(*it2)->getX(),(*it2)->getY(),ENNEMI_WIDTH,ENNEMI_HEIGHT,true,false);
		}	
	}
...
{
	if (_musique_de_fond.GetStatus()!=sf::Sound::Playing)
