...
{}
//==============================
EnnemiSprite::EnnemiSprite(Image *image, int x, int y, int w, int h,bool anime, bool explose)	:  AnimeGraphicElement(image,x,y,w,h)
{
	_anime = anime;
	_explose=explose;
	_buffer.LoadFromFile("musiques/dragon_mort.wav");
	_sound.SetBuffer(_buffer);	//on set le son selon celui donné en parametres
}
...
void EnnemiSprite::draw(RenderWindow * _window)
{
	...
	if(_explose)
	{
		...
		if(_clock.GetElapsedTime() <=0.3)		
				SetSubRect(sf::IntRect(0, 100, 150, 200));
		else if (_clock.GetElapsedTime() <=0.6)
			SetSubRect(sf::IntRect(150, 100, 300, 200));
		else if (_clock.GetElapsedTime() <=0.8)
