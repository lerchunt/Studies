...
GameView::GameView(int w, int h, int bpp): _w(w), _h(h)
{
	...
	_buffer2.LoadFromFile("musiques/dragon_mort.wav");		//source du son du dragon
	_buffer.LoadFromFile("musiques/tir_sorciere.wav"); 		// source du son du tir de la sorcière
	_buffer_secondaire.LoadFromFile("musiques/tir_secondaire.wav");
	_sound.SetBuffer(_buffer);								// on associe le buffer au son
	_sound_secondaire.SetBuffer(_buffer_secondaire);
...
void GameView::treatEventsJeu(GameModel*model)
{
	...
	{
			if(model->getJoueur()->getTirerSecondaire())
			_sound_secondaire.Play();
			_model->creerTirSecondaire();			//on demande au modele de creer le tir
			_clock.Reset();
		}
