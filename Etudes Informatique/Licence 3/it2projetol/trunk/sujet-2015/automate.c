...
/*echange l'origine et la fin d'une transition'*/
void echanger_origine_fin( int origine, char lettre, int fin, void* data ){
	ajouter_transition(data, fin, lettre, origine);
}


Automate *miroir( const Automate * automate){
	/*On échange le contenu des ensembles initiaux et finaux en créant un nouvel automate*/
	Automate * new_aut = creer_automate();
	new_aut->initiaux = copier_ensemble(automate->finaux);
	new_aut->finaux = copier_ensemble(automate->initiaux);
	new_aut->etats = copier_ensemble(automate->etats); 
	pour_toute_transition(automate, echanger_origine_fin, new_aut);
	return new_aut;
}
...
Automate * creer_automate_minimal( const Automate* automate ){
    // on déterminise à deux reprises en ayant fait un miroir préalablement pour obtenir le minimal
    Automate * min=miroir(automate);
    min=creer_automate_deterministe(min);
    min=miroir(min);
    min=creer_automate_deterministe(min);
